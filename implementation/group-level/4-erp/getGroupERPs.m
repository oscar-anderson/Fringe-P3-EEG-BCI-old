function groupLevelERPs = getGroupERPs(subjectDataset)

groupLevelERPs = struct();
groupLevelERPs.info = subjectDataset.info;
        ...
        for iSet = 1:numMorphSets
            ...
            for iSubject = 1:numSubjects
                ...
                if isfield(subjectDataset.(subjectField).(blockField).(conditionField), sprintf('%s', morphField))
                    groupLevelERPs.(blockField).(conditionField).(morphField){iSubject} ...
